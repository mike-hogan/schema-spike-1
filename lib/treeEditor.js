...
ExistingObjectNodeAccessor.prototype.removeExistingNode = function(document) {
    if(this.nextPathElements.length === 0) {
        ...
        const nodeAccessor = _existingNodeAccessor(nextNodeName, nextSchemaNode, _.rest(this.nextPathElements));
...
const _createExistingNodeAccessor = function(path, schema) {
    path = path.replace(/^\//, "");
    const pathParts = path.split("/");
    const rootNodeName = _.first(pathParts);
    const rootSchemaNode = schema[rootNodeName];
    return _existingNodeAccessor(rootNodeName, rootSchemaNode, _.rest(pathParts));
}

TreeEditor.prototype.setExistingNode = function(document,path, newNode) {
    const nodeAccessor = _createExistingNodeAccessor(path, this.schema);
